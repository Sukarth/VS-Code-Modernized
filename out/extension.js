 ...
 */
async function findWorkbenchPath() {
 ...
 */
async function writeFileWithPermissions(targetUri, content) {
    try {
        ...
        try {
            ...
            return await new Promise((resolve) => {
                sudo.exec(command, { name: 'VS Code Modernized Extension' }, async (sudoError) => {
                    if (sudoError) {
                        ...
                        vscode.window.showErrorMessage(`Failed to apply styles due to permission error. Please try running VS Code as an administrator/root. Error: ${sudoError}`);
...
function generateDynamicCss() {
    ...
    const spacing = config.get('ui.spacing', '0.35rem');
    const borderRadius = config.get('ui.borderRadius', '0.5rem');
    const tabSpacing = config.get('ui.tabSpacing', '0.2rem');
    // Basic validation to ensure the value looks like a valid CSS unit
    const isValidUnit = (value) => /^\d+(\.\d+)?(rem|em|px|%|vh|vw|vmin|vmax)$/.test(value);
    return `
...
`;
        ...
        // --- Theme Handling ---
        const config = vscode.workspace.getConfiguration(CONFIG_SECTION);
        const autoApplyTheme = config.get('autoApplyTheme', true);
...
async function removeStyles() {
    ...
    try {
        ...
        // --- Theme Handling ---
        const currentTheme = vscode.workspace.getConfiguration('workbench').get('colorTheme');
        const previousTheme = extensionContext.globalState.get(PREVIOUS_THEME_KEY);
 ...
 */
async function generateDebugReport(vscodeRoot, possiblePaths) {
    let fileStructure = [];
    ...
    try {
        const stats = await promises_1.default.stat(vscodeRoot.fsPath);
    ...
    try {
        ...
        if (!vsFound) {
            ...
            fileStructure.push('out/', ...outFiles.map(([name, type]) => `    ${name}${type === vscode.FileType.Directory ? '/' : ''}`));
        }
        else {
            ...
            try {
                ...
                if (!codeFound) {
                    ...
                    fileStructure.push('vs/', ...vsFiles.map(([name, type]) => `    ${name}${type === vscode.FileType.Directory ? '/' : ''}`));
                }
                else {
                    ...
                    try {
                        ...
                        fileStructure.push('code/', ...codeFiles.map(([name, type]) => `    ${name}${type === vscode.FileType.Directory ? '/' : ''}`));
                    }
                    catch { }
                }
            }
...
async function activate(context) {
    ...
    const pendingThemeChange = context.globalState.get('pendingThemeChange');
    const pendingThemeRevert = context.globalState.get('pendingThemeRevert');
    ...
    if (!workbenchHtmlPath) {
        vscode.window.showWarningMessage(`Could not locate VS Code's workbench.html file. Style injection commands will not work.`);
    ...
    // --- Register Commands ---
    context.subscriptions.push(vscode.commands.registerCommand(`${CONFIG_SECTION}.applyStyles`, applyStyles), vscode.commands.registerCommand(`${CONFIG_SECTION}.removeStyles`, removeStyles));
    ...
    const stylesAreApplied = context.globalState.get(APPLIED_STATE_KEY, false);
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration(event => {
        ...
        if (affectsOurUi) {
            ...
            const currentlyApplied = extensionContext.globalState.get(APPLIED_STATE_KEY, false);
            if (currentlyApplied) {
